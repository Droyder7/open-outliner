...
/**
 * The Hocuspocus collaboration server (COL) wired to the ack-after-persist store
 * (ADR-0015) and the debounced materializer (ADR-0013).
 *
 ...
 *
 * Auth (`onAuthenticate`) is wired in Phase 5 (SEC).
 */
...
export function createCollabServer(deps: CollabServerDeps): CollabServer {
  ...
  return { hocuspocus, store, projector };
}
