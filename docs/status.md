...
| `API` | API and non-CRDT write path | V1 | Implemented | Single `POST /rpc` endpoint dispatching typed commands over the shared command/error envelope ([rpc.ts](../packages/shared/src/rpc.ts), [dispatch.ts](../packages/server/src/rpc/dispatch.ts), [handlers.ts](../packages/server/src/rpc/handlers.ts), [http/server.ts](../packages/server/src/http/server.ts)): `GetItems`/`GetChildren` load-on-expand via a depth-bounded recursive CTE ([items-repo.ts](../packages/server/src/db/items-repo.ts)), `UpdateItem` optimistic concurrency (`version`) distinguishing `version_conflict` from `not_found`, `CreateDocument`/`ListDocuments`/`InviteMember`/`RemoveMember`/`WhoAmI`. `PresignUpload`/`PresignDownload` are typed but deliberately return a typed `internal` stub — S3/MinIO wiring is Phase 8 (`ATT`). Client consumes it through a typed `createRpcClient` (cookie session + CSRF header, [rpc.ts](../packages/client/src/rpc.ts)) behind a minimal Signup/Login gate in server-synced mode ([AuthGate.tsx](../packages/client/src/auth/AuthGate.tsx)). Gate met: command round-trip + typed-error + version-conflict tests pass against real Postgres ([rpc.integration.test.ts](../packages/server/test/rpc.integration.test.ts), [http-server.integration.test.ts](../packages/server/test/http-server.integration.test.ts)); real process boot smoke-tested end to end (`main.ts`). Remaining before Verified: attachment presign (Phase 8), a workspace/document picker UI (client currently opens `?doc=` directly), browser e2e. |
